...
def main():
    ...
    # Focus on Fragrance Oils
    fragrance_oils = [p for p in products if "fragrance oil" in p.get("category", "").lower() or "fragrance" in p.get("name", "").lower()]
    print(f"Total fragrance oils in JSON: {len(fragrance_oils)}")
    ...
    for p in fragrance_oils:
        full_name = p["name"]
        base_name = full_name.split("(")[0].replace("Fragrance Oil", "").strip()
    ...
    for img_path in gemini_files:
        try:
            ...
            if best_match:
                base_name = best_match["name"].split("(")[0].replace("Fragrance Oil", "").strip()
                ...
                best_match["image"] = f"/images/products/{target_filename}"
                matched_count += 1
